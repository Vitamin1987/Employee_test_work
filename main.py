...
def read_csv_file(file_path: str) -> List[Dict[str, Any]]:
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            if not headers:
                raise PayrollError(
                    f"Файл {file_path} пустой или не содержит заголовков"
                )
            ...
            if not required_fields.issubset(header_set):
                ...
                raise PayrollError(
                    f"В файле {file_path} отсутствуют обязательные поля: {missing_fields}"
                )
            ...
            for line_num, line in enumerate(f, start=2):
                ...
                if len(values) != len(headers):
                    raise PayrollError(
                        f"Некорректный формат строки {line_num} в файле {file_path}"
                    )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Скрипт для формирования отчётов по сотрудникам."
    )
    parser.add_argument(
        'files', nargs='+', action=FileExistsAction,
        help="Пути к CSV-файлам с данными сотрудников."
    )
    parser.add_argument(
        '--report', required=True, choices=list(REPORTS.keys()),
        ...
        try:
            ...
            for data in employees_data:
                try:
                    ...
                    raise PayrollError(
                        f"Ошибка в данных файла {file_path}: {e}"
                    )
        except PayrollError as e:
            print(f"Ошибка: {e}")
            sys.exit(1)
